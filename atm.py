...
def query_dict_for_account(user_query, dict):
	if user_query in dict.keys():
		my_user_query = BankAccount(user_query, dict[user_query][0], dict[user_query][1])
		return my_user_query
	else: print("We don't have a listing for you. Sorry, we're new at this banking thing.")

def atm_menu(loop_bool_variable, dict_of_atm_users):
	...
	os.system('clear')
	user_go_again = input("Continue?: ")
	if user_go_again.lower() == 'n': loop_bool_variable = False

	return loop_bool_variable
...
while loop_bool:
	loop_bool = atm_menu(loop_bool, dict_of_atm_users)
